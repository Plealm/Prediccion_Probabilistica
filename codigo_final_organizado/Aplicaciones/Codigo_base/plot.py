...
def plot_type_a_vertical_stack(group_name, models_to_plot, predictions_dict, df_results, 
    ...
    for i, t in enumerate(steps):
        ...
        ax.set_title(f"Step {step_num} | Real: {true_val:.2f}", fontsize=12, fontweight='bold', loc='left')
        ax.set_ylabel("Densidad")
        ax.grid(True, alpha=0.3)
        
        # Leyenda dentro
        full_txt = "CRPS:\n" + "\n".join(legend_labels)
        at = AnchoredText(full_txt, prop=dict(size=9), frameon=True, loc='upper right')
...
def plot_type_b_step_comparison(target_step_idx, predictions_dict, df_results, 
    ...
    for i, m in enumerate(sorted_models):
        ...
        ax.set_title(m, loc='left', fontsize=11, fontweight='bold', color=color)
        ax.text(0.99, 0.8, crps_txt, transform=ax.transAxes, ha='right',

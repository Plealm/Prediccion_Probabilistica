...
class PipelineElectricityRolling(PipelineElectricity):
    """
    Pipeline para Electricity donde los modelos RE-ENTRENAN sus parámetros 
    en cada paso de tiempo (Rolling Window / No-Frozen).
    """

    def _optimize_hyperparameters_only(self, models, train_data, val_data):
        """
        Fase 1: Solo busca los mejores hiperparámetros (b1, b2).
        NO congela parámetros ni entrena el modelo definitivo aquí.
        """
        if self.verbose:
            print("\n🔍 Optimizando hiperparámetros (Fase de Validación)...")
        
        optimizer = TimeBalancedOptimizerWithTQDM(random_state=self.seed, verbose=self.verbose)
        optimized_params = optimizer.optimize_all_models(models, train_data, val_data)
        
        # Aplicamos los hiperparámetros encontrados a los modelos
        for name, params in optimized_params.items():
            if params and name in models:
                model = models[name]
    ...
    def run_evaluation(self, series_index=0, save_predictions=False):
        print("="*60)
        print(f"🔌 ELECTRICITY ROLLING (RE-ENTRENAMIENTO PASO A PASO) - Serie {series_index}")
        print("="*60)
        ...
        # 2. OPTIMIZAR HIPERPARÁMETROS (Encontrar b*)
        # En este punto los modelos NO están 'frozen'.
        models = self._optimize_hyperparameters_only(models, split['train']['values'], split['val']['values'])
        
        if self.verbose: 
            print(f"\n🔮 Fase 3: Predicción Rolling ({self.N_TEST_HOURS} pasos)")
            print("Nota: Los modelos re-estimarán parámetros (c) en cada paso usando b*.")

        results_rows = []
        predictions_dict = {} if save_predictions else None
        
        test_vals = split['test']['values']
        # La historia inicial incluye Train + Val
        history = np.concatenate([split['train']['values'], split['val']['values']])
        
        for t in tqdm(range(len(test_vals)), desc="Rolling Predict"):
            # En cada paso, la historia crece
            curr_hist = np.concatenate([history, test_vals[:t]])
            true_val = test_vals[t]
            row = {'Paso': t+1, 'Valor_Observado': true_val}
            ...
            if save_predictions:
                predictions_dict[t] = {
                    'timestamp': split['test']['timestamps'][t], 
                    'true_value': true_val, 
                    'predictions': {}
                }
            ...
            for name, model in models.items():
                try:
                    # ASEGURAR QUE NO ESTÉ CONGELADO
                    # (En modelos.py, si _is_frozen es False, fit_predict re-entrena)
                    if hasattr(model, '_is_frozen'):
                        model._is_frozen = False
                    
                    # El modelo ejecuta su lógica completa de ajuste (fit) con curr_hist
                    if isinstance(model, (CircularBlockBootstrapModel, SieveBootstrapModel)):
                        preds = model.fit_predict(curr_hist)
                    else:
                        # Para DeepAR y EnCQR, esto gatilla el entrenamiento de 'epochs' en cada paso t
                        preds = model.fit_predict(pd.DataFrame({'valor': curr_hist}))
                    ...
                    # En rolling, los errores son más comunes por falta de convergencia en pasos cortos
                    row[name] = np.nan
            
            results_rows.append(row)
            
            # Limpieza agresiva de memoria (vital en rolling porque cada paso crea grafos de TF/Torch)
            if t % 2 == 0: 
                clear_all_sessions()

...
La implementación desarrollada traduce los cuantiles ponderados de \textcite{Barber2023} a un contexto de distribuciones predictivas completas:

\paragraph{Adaptación 1: Expansión Ponderada de Distribuciones}
La teoría se enfoca en calcular cuantiles específicos. Para evaluar ECRPS (que requiere la distribución completa), la implementación genera una muestra sintética de tamaño fijo ($N = 1000$) mediante \textit{weighted expansion}: cada valor crítico $C_i$ se replica proporcionalmente a su peso:
\begin{equation}
\text{replications}_i = \lfloor \tilde{w}_i \times N \rfloor
\end{equation}

Esto produce una distribución empírica discreta donde la frecuencia relativa de $C_i$ aproxima $\tilde{w}_i$.

\paragraph{Adaptación 2: Ajuste de Replicaciones Enteras}
Como $\tilde{w}_i \times N$ rara vez es entero exacto, se implementa un algoritmo de ajuste que garantiza $\sum_i \text{replications}_i = N$ exactamente:
\begin{enumerate}
    \item Se redondea cada $\tilde{w}_i \times N$ al entero más cercano.
    \item Si la suma excede $N$, se decrementa la replicación de los valores con menor peso (manteniendo mínimo 1).
    \item Si la suma es menor a $N$, se incrementa la replicación de los valores con mayor peso.
\end{enumerate}

Este ajuste minimiza la distorsión de la distribución ponderada original.

\paragraph{Adaptación 3: Congelamiento de Estructura Ponderada}
A diferencia de un esquema rolling que recalcularía pesos en cada ventana, la implementación congela tres artefactos tras la calibración:
\begin{itemize}
    \item \texttt{\_frozen\_rho}: Valor óptimo $\rho^*$ seleccionado por validación
    \item \texttt{\_frozen\_critical\_values}: Vector de residuos conformales $\{C_1, \dots, C_m\}$
    \item \texttt{\_frozen\_weights}: Vector de pesos $\{\tilde{w}_1, \dots, \tilde{w}_m\}$
\end{itemize}

En cada predicción rolling subsecuente, se reutilizan estos artefactos sin recalcular la matriz hat ni ajustar OLS, generando la distribución predictiva exclusivamente mediante expansión ponderada de los valores críticos congelados.
...
Pesos & Definición abstracta $w_i$ & Expansión por replicación entera \\
Actualización & Recalcular en cada predicción & Artefactos congelados post-calibración \\
Residuos & Conceptuales & Valores críticos LSPM studentizados \\
...
\paragraph{Métrica de Optimización}
La selección de $\rho^*$ se realiza minimizando el \textbf{ECRPS} (véase \ref{subsec:ecrps}) sobre el conjunto de validación. Para cada candidato $\rho$, se genera la distribución ponderada en cada tiempo de validación y se evalúa la calibración predictiva.

\paragraph{Protocolo de Congelamiento}
El método \texttt{freeze\_hyperparameters()} ejecuta:
\begin{enumerate}
    \item Congela $p^*$ (número de lags) heredado de la clase base LSPM.
    \item Calcula valores críticos $\{C_1, \dots, C_m\}$ sobre datos de entrenamiento+calibración.
    \item Computa pesos temporales usando $\rho^*$: $w_i = (\rho^*)^{m-1-i}$ para $i = 1, \dots, m$.
    \item Normaliza pesos: $\tilde{w}_i = w_i / \sum_j w_j$.
    \item Almacena artefactos en atributos internos y activa bandera \texttt{\_is\_frozen}.
\end{enumerate}

Durante la fase de prueba rolling, el método \texttt{fit\_predict()} usa exclusivamente los artefactos congelados para generar distribuciones predictivas, sin recalcular residuos ni ajustar modelos OLS.
...
\begin{itemize}
    ...
    \item \texttt{random\_state}: Semilla para shuffle final de la distribución expandida (rompe ordenamiento artificial).
...
\label{subsec:simulaciones_complementarias}

Además del diseño factorial principal, se implementaron cinco conjuntos de simulaciones complementarias diseñadas para resolver dilemas metodológicos sobre el preprocesamiento, la persistencia de los datos, la arquitectura de la muestra y la propagación de la incertidumbre en horizontes lejanos.

\subsection{Simulación 1: Impacto de la Diferenciación en ARIMA ($d=1$)}
\label{subsubsec:sim_diferenciacion}

\paragraph{Motivación}
Esta simulación evalúa si los métodos conformales capturan mejor la variabilidad cuando operan sobre la serie estacionaria ($\Delta Y_t$) o sobre los niveles integrados ($Y_t$). 

\paragraph{Diseño Experimental}
Se utilizan 7 configuraciones ARIMA con $d=1$ (7 procesos base $\times$ 5 distribuciones $\times$ 4 varianzas). Cada escenario se ejecuta bajo dos modalidades:
\begin{itemize}
    \item \textbf{SIN\_DIFF:} El modelo recibe $Y_t$ y genera una distribución predictiva para $Y_{t+1}$.
    \item \textbf{CON\_DIFF:} El modelo recibe $\Delta Y_t$. La predicción generada $\widehat{\Delta Y}_{t+1}$ se integra mediante $\hat{Y}_{t+1} = Y_t + \widehat{\Delta Y}_{t+1}$ para calcular el ECRPS en el espacio de niveles.
\end{itemize}

\subsection{Simulación 2: Límites de Integración y Persistencia (Multi-D)}
\label{subsubsec:sim_multi_d}

\paragraph{Motivación}
Se investiga la estabilidad numérica de los métodos conformales ante órdenes de integración elevados $d \in \{1, 2, 3, 4, 5, 6, 7, 10\}$.

\paragraph{Diseño Experimental}
A partir de un proceso estacionario $W_t$, se genera $Y_t$ mediante la aplicación sucesiva del operador de integración $d$ veces. Al igual que en la Simulación 1, se comparan las modalidades \textbf{SIN\_DIFF} y \textbf{CON\_DIFF}. Se hipotetiza que para $d > 3$, el rango de los datos en niveles desestabilizará a los modelos que no utilicen diferenciación previa.

\subsection{Simulación 3: Efectos del Tamaño Muestral Absoluto}
\label{subsubsec:sim_tamano_muestral}

\paragraph{Motivación}
Caracteriza la tasa de convergencia de las distribuciones predictivas empíricas hacia la densidad teórica a medida que el volumen de datos aumenta.

\paragraph{Diseño Experimental}
Se evalúan procesos ARMA, ARIMA y SETAR bajo tres escalas:
\begin{itemize}
    \item \textbf{Pequeña:} $n_{\text{train}} = 100$, $n_{\text{cal}} = 20$ ($n_{\text{total}} = 120$).
    \item \textbf{Media:} $n_{\text{train}} = 500$, $n_{\text{cal}} = 100$ ($n_{\text{total}} = 600$).
    \item \textbf{Grande:} $n_{\text{train}} = 1000$, $n_{\text{cal}} = 200$ ($n_{\text{total}} = 1200$).
\end{itemize}

\subsection{Simulación 4: Proporciones de Calibración con Tamaño Fijo}
\label{subsubsec:sim_proporciones}

\paragraph{Motivación}
En escenarios de datos limitados, existe un conflicto entre usar datos para mejorar el ajuste del modelo (\textit{training}) o para mejorar la precisión de los intervalos (\textit{calibration}).

\paragraph{Diseño Experimental}
Se fija un presupuesto total de $n_{\text{total}} = 240$ observaciones. Se evalúan 5 particiones de calibración: $10\%, 20\%, 30\%, 40\%$ y $50\%$ de la muestra total. El objetivo es determinar si existe una "proporción áurea" que minimice el ECRPS promedio en todas las familias de procesos.

\subsection{Simulación 5: Predicción Multi-paso (Horizonte $h$)}
\label{subsubsec:sim_multi_paso}

\paragraph{Motivación}
Evalúa cómo se degrada la calidad de la distribución predictiva cuando el modelo debe predecir de forma recursiva, alimentándose de sus propias salidas previas.

\paragraph{Diseño Experimental}
A diferencia del diseño principal (ventana rodante a un paso), aquí se proyectan $H=12$ pasos hacia adelante desde un origen fijo. Para cada uno de los 4 modelos evaluados (LSPM, DeepAR, Sieve Bootstrap, MCPS), se generan $N_{\text{traj}} = 100$ trayectorias estocásticas:
\begin{equation}
\hat{Y}_{t+h}^{(m)} \sim F_{\text{modelo}}(\cdot \mid Y_{1:t}, \hat{Y}_{t+1}^{(m)}, \dots, \hat{Y}_{t+h-1}^{(m)})
\end{equation}
La distribución de estas trayectorias en cada horizonte $h$ se compara mediante ECRPS contra la distribución teórica real generada por el DGP mediante 1,000 muestras de Monte Carlo.

\subsection{Resumen de Evaluaciones Complementarias}

La Tabla \ref{tab:resumen_complementarias} resume el alcance total de estos experimentos.
...
\toprule
\textbf{Simulación} & \textbf{Factor Variado} & \textbf{Evaluaciones} \\
\midrule
1. Diferenciación ($d=1$) & Modalidad (SIN\_DIFF vs CON\_DIFF) & 3,360 \\
2. Multi-D & Orden de integración $d \in \{1 \dots 10\}$ & 26,880 \\
3. Tamaño Muestral & Escala total ($n=120, 600, 1200$) & 4,320 \\
4. Proporciones & Ratio $n_{\text{cal}} / n_{\text{train}}$ (Total 240 fijo) & 25,200 \\
5. Multi-paso & Horizonte de predicción $h \in \{1 \dots 12\}$ & 4,320 \\
\midrule
\textbf{Total} & & \textbf{64,080} \\
\bottomrule
\end{tabular}
\end{table}
...
\item \textbf{Conjunto de Calibración:} $n_{\text{cal}} = 40$ observaciones subsecuentes utilizadas para la calibración de los hiperparámetros y la construcción de distribuciones conformales.
...
\item Este esquema emula una situación operativa donde el analista actualiza periódicamente los modelos conforme nueva información queda disponible.
\end{itemize}
...
\end{equation}
donde $\Phi(L) = 1 - \sum_{i=1}^p \phi_i L^i$ es el polinomio autorregresivo y $\Theta(L) = 1 + \sum_{j=1}^q \theta_j L^j$ es el polinomio de media móvil suponiendo que los polinomios $\Phi(L)$ y $\Theta(L)$ no tienen raíces comunes.
...
Para un proceso ARMA estacionario e invertible, la distribución del siguiente valor $Y_{n+1}$ condicionada en la historia observada $\mathcal{F}_n = \{Y_1, \ldots, Y_n, \varepsilon_1, \ldots, \varepsilon_n\}$ que tiene una forma analítica explícita: dado que el modelo es lineal, la distribución condicional queda completamente caracterizada por su media condicional y su varianza condicional, calculadas directamente de la ecuación estructural.
...
\paragraph{Caso SETAR(2; 1, 1):} Para el caso más simple de dos regímenes con orden autorregresivo 1, \textcite{petruccelli1984consistent} demostraron que el proceso es ergódico si y solo si:
\begin{equation}
...
\paragraph{\texorpdfstring{Caso General SETAR(2; $p_1$, $p_2$):}{Caso General SETAR(2; p1, p2):}} Para órdenes autorregresivos mayores,\textcite{chan1985testing} proporcionaron una condición suficiente basada en el radio espectral de las matrices compañeras:
\begin{equation}
...
Mientras que la teoría requiere especificación manual del tamaño de bloque basado en análisis del proceso estocástico subyacente, la implementación incorpora la heurística automática de \textcite{politis2004automatic}: $l \approx 1.5 \times n^{1/3}$, además de otros valores de referencia que equilibran eficiencia estadística y 
captura de dependencia temporal.
...
\begin{enumerate}
    ...
    \item Establece el indicador de congelamiento.
\end{enumerate}
...
El método ejecuta el siguiente protocolo:
\begin{enumerate}
...
\textbf{(4) Protocolo de Congelamiento Bidimensional:} El método fija simultáneamente los siguientes parámetros:
...
Para establecer la significancia estadística de las diferencias observadas entre pares de métodos, se emplea el test de Diebold-Mariano modificado (ecuación \eqref{eq:dm_modified}) con la corrección para muestras pequeñas propuesta por \textcite{Harvey1997}. Para cada escenario, se calculan los diferenciales de pérdida $d_t = \text{ECRPS}(F_A^{(t)}, G^{(t)}) - \text{ECRPS}(F_B^{(t)}, G^{(t)})$ utilizando los $h=12$ horizontes de pronóstico, y el estadístico resultante se compara contra la distribución $t$ de Student con $(h-1)$ grados de libertad.

Dado que se realizan múltiples comparaciones entre los nueve métodos evaluados, se aplica la corrección de Bonferroni al nivel de significancia: si se contrastan $K$ pares de métodos al nivel global $\alpha = 0.05$, cada test individual se realiza al nivel $\alpha/K$ para controlar la tasa de error tipo I familiar. Este procedimiento genera un $p$-valor para cada combinación de escenario y par de modelos.

Para facilitar la interpretación de los resultados, se construyen matrices de resumen $n \times n$ (donde $n$ es el número de modelos) que reportan dos métricas: (i) el porcentaje de escenarios en los que el modelo de la fila supera significativamente al modelo de la columna, y (ii) el porcentaje de escenarios en los que el ECRPS promedio del modelo de la fila es menor que el del modelo de la columna. 

En escenarios con características adicionales (e.g., orden de diferenciación $d$, tamaño muestral, proporción de datos), el análisis se desagrega mediante matrices $n \times m$, donde $m$ representa el número de niveles de la característica evaluada. Cada celda de estas matrices presenta el porcentaje de escenarios en los que el modelo correspondiente alcanza significancia estadística en ese nivel específico, junto con su ECRPS promedio.
...
Para cada método evaluado, se calculan los valores $\text{CRPS}(F_t, y_t)$ a lo largo de la ventana de evaluación de $T$ pasos. Las comparaciones entre métodos se realizan mediante el test de Diebold-Mariano modificado, donde los diferenciales de pérdida $d_t = \text{CRPS}(F_A^{(t)}, y_t) - \text{CRPS}(F_B^{(t)}, y_t)$ para $t = 1, \ldots, T$ constituyen las observaciones pareadas del test. El estadístico resultante se compara contra la distribución $t$ de Student con $(T-1)$ grados de libertad, aplicando la misma corrección de Bonferroni utilizada en los escenarios simulados para controlar la tasa de error tipo I en comparaciones múltiples.
...
Esta metodología integrada proporciona una evaluación exhaustiva que combina precisión puntual (vía CRPS), calibración probabilística (vía PIT y curvas de confiabilidad) y comparaciones estadísticamente rigurosas (vía tests de Diebold-Mariano con corrección de Bonferroni).
...
% ----------------------------------------------------------------------------
\subsection{\texorpdfstring{Simulación 1: Impacto de la Diferenciación en ARIMA ($d=1$)}{Simulación 1: Impacto de la Diferenciación en ARIMA (d=1)}}
...
\subsection{\texorpdfstring{Simulación 5: Predicción Multi-paso (Horizonte $h$)}{Simulación 5: Predicción Multi-paso (Horizonte h)}}
...
La Tabla~\ref{tab:resumen_complementarias} consolida el alcance total de estos experimentos. El volumen combinado de estas simulaciones complementarias es comparable al del diseño principal, reflejando la importancia de estos aspectos metodológicos para una evaluación exhaustiva.

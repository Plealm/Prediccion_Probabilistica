...
\begin{table}[htbp]
\centering
\caption{Ranking de modelos según desempeño en dataset Exchange Rate. Se reportan media y mediana del CRPS, además de métricas de comparación pareada. Valores menores de CRPS indican mejor desempeño.}
...
\begin{figure}[htbp]
    ...
    \caption{Distribución de valores CRPS por modelo en el conjunto de prueba del dataset Exchange Rate. La caja representa el rango intercuartílico, la línea central indica la mediana, y los puntos individuales muestran valores atípicos. Nótese la clara separación entre el cluster superior de seis modelos competitivos y el grupo inferior de tres métodos inadecuados para esta serie.}
...
\begin{table}[htbp]
\centering
\caption{Matriz de valores p del test de Diebold-Mariano modificado para el dataset Exchange Rate. Valores menores a 0.05 (resaltados en negrita) indican diferencias estadísticamente significativas en capacidad predictiva al nivel de confianza del 95\%.}
...
\begin{figure}[htbp]
    ...
    \caption{Histogramas de transformación PIT para todos los modelos en el dataset Exchange Rate. Una distribución uniforme (línea horizontal punteada) indica calibración perfecta. La concentración extrema de masa en las colas para varios modelos refleja la dificultad de cuantificar incertidumbre en presencia de heterocedasticidad condicional severa.}
...
\begin{figure}[htbp]
    ...
    \caption{Curvas de confiabilidad para los modelos evaluados en el dataset Exchange Rate. La diagonal representa calibración perfecta. Las trayectorias escalonadas reflejan el tamaño reducido del conjunto de prueba (24 observaciones). Nótese el colapso completo de calibración para EnCQR-LSTM, Block Bootstrapping y AREPD en niveles de confianza altos.}

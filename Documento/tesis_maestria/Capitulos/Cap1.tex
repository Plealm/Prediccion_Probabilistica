...
La evaluación mediante el promedio del \textit{Continuous Ranked Probability Score} (ECRPS) y pruebas de significancia estadística como el test de Diebold-Mariano permitirán establecer comparaciones robustas. Esta comparación sistemática con métodos estándar (Bootstrapping) y estado del arte (\textit{DeepAR}) proporcionará una perspectiva clara sobre las fortalezas y debilidades del enfoque conformal en escenarios reales y simulados, facilitando la selección informada de métodos de pronóstico probabilístico en aplicaciones prácticas.
\section{Objetivos}

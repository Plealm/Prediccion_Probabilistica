...
El pronóstico probabilístico representa un cambio de paradigma fundamental en la predicción estadística, pasando de estimaciones puntuales a distribuciones de probabilidad completas sobre cantidades futuras de interés \parencite{Gneiting2014}. A diferencia de las predicciones puntuales tradicionales, que proporcionan únicamente un valor esperado o una estimación central,
el pronóstico probabilístico cuantifica la incertidumbre asociada a la predicción mediante la especificación de una distribución predictiva completa \parencite{GneitingBalabdaoui2007}.
...
Gneiting y Raftery explican que el objetivo fundamental del pronóstico probabilístico es maximizar la nitidez de las distribuciones predictivas sujeto a calibración. A continuación se caracterizan formalmente:
...
Una \textit{regla de puntuación} (\textit{scoring rule}) es una función $S: \mathcal{F} \times \mathbb{R} \to \mathbb{R} \cup \{-\infty, \infty\}$ que asigna una penalización numérica $S(F,y)$ a cada par formado por una distribución predictiva $F \in \mathcal{F}$ y una observación realizada $y$ \parencite{GneitingRaftery2007}. En la notación empleada, valores más bajos de la puntuación indican un mejor desempeño predictivo.
...
\end{equation}
donde $F$ es la función de distribución acumulada (FDA) de la distribución predictiva y $y$ es la observación realizada. Esta representación muestra que el CRPS equivale al área cuadrática entre la FDA predictiva $F$ y la función escalonada $\mathbbm{1}\{y \leq \cdot\}$, que corresponde a la distribución degenerada concentrada en $y$ \parencite{Thorarinsdottir2017}.
...
\subsection{Enfoque de la Teoría Asintótica de Suavizado Fijo}
...
Cuando se realizan comparaciones múltiples entre métodos de pronóstico, surge el problema del error tipo I acumulado: la probabilidad de rechazar erróneamente al menos una hipótesis nula verdadera aumenta con el número de tests realizados. Si se efectúan $K$ comparaciones independientes, cada una al nivel de significancia $\alpha$, la probabilidad de cometer al menos un error tipo I puede alcanzar valores sustancialmente superiores a $\alpha$ \parencite{Dunn1961}.

La corrección de Bonferroni \parencite{Bonferroni1936} constituye el método más conservador y ampliamente utilizado para controlar la tasa de error familiar (Family-Wise Error Rate, FWER), definida como la probabilidad de rechazar incorrectamente al menos una hipótesis nula verdadera entre el conjunto de hipótesis evaluadas. El procedimiento ajusta el nivel de significancia individual de cada test según:
\begin{equation}
\alpha_{\text{individual}} = \frac{\alpha_{\text{global}}}{K}
\label{eq:bonferroni}
\end{equation}
donde $\alpha_{\text{global}}$ representa el nivel de significancia deseado para el conjunto completo de comparaciones (típicamente 0.05) y $K$ denota el número total de tests realizados.

Bajo esta corrección, la FWER queda acotada superiormente por $\alpha_{\text{global}}$ mediante la desigualdad de Boole. Específicamente, si $P_i$ representa el valor-p del test $i$-ésimo, la hipótesis nula correspondiente se rechaza únicamente si $P_i \leq \alpha_{\text{global}}/K$. Este ajuste garantiza que:
\begin{multline}
\text{FWER} = P\left(\bigcup_{i=1}^K \{\text{rechazar } H_{0,i} \mid H_{0,i} \text{ verdadera}\}\right) \\
\leq \sum_{i=1}^K P(\text{rechazar } H_{0,i} \mid H_{0,i} \text{ verdadera}) \\
\leq \alpha_{\text{global}}
\end{multline}

En el contexto de este trabajo, donde se comparan nueve métodos de pronóstico probabilístico, el número de comparaciones pareadas asciende a $K = \binom{9}{2} = 36$. Aplicando la corrección de Bonferroni con $\alpha_{\text{global}} = 0.05$, cada test de Diebold-Mariano individual se evalúa al nivel $\alpha_{\text{individual}} = 0.05/36 \approx 0.0014$. Si bien este ajuste reduce la potencia estadística de cada test individual, proporciona garantías rigurosas contra conclusiones espurias derivadas de comparaciones múltiples, aspecto fundamental cuando se busca identificar diferencias genuinas en el desempeño predictivo entre metodologías competidoras.

\section{Predicción Conformal por Intervalos: El Enfoque IID}
\label{sec:prediccion_conformal_intervalos_IIE}

La predicción conformal clásica, introducida por \textcite{Vovk2005}, se fundamenta en la capacidad de generar conjuntos de predicción $\Gamma^\epsilon$ que garantizan una cobertura de confianza exacta para cualquier nivel de significancia $\epsilon \in (0,1)$. A diferencia de los métodos estadísticos tradicionales que dependen de la teoría asintótica (grandes muestras), la predicción conformal es válida para muestras finitas, siempre que se cumpla el supuesto de intercambiabilidad de los datos.
...
Para construir un intervalo de predicción para un nuevo objeto $x_n$, se define $z_i = (x_i, y_i)$, lo que permite representar el conjunto de entrenamiento como $z_1, \dots, z_{n-1}$, sobre el cual se realiza un proceso de prueba de hipótesis inversa. Para cada valor potencial $y \in \mathbb{R}$:
...
\subsection{\texorpdfstring{La Taxonomía de Mondrian ($\kappa$)}{La Taxonomía de Mondrian (kappa)}}
...
Esta propiedad implica que, asintóticamente, el CPS converge a la verdadera distribución generadora de los datos con precisión arbitraria, de modo que la incertidumbre epistémica tiende a desvanecerse conforme $n \to \infty$ \parencite{Vovk2019}. 

...
Esta sección presenta los resultados del diseño  completo descrito en la Sección~\ref{sec:diseño_experimental}, que evalúa 9 métodos bajo 420 configuraciones únicas distribuidas entre tres escenarios (ARMA, ARIMA, SETAR). El análisis se estructura en dos niveles: primero se examinan los patrones agregados que emergen del conjunto completo de simulaciones, y posteriormente se descomponen los resultados por escenario para identificar comportamientos específicos asociados a cada clase de proceso generador de datos. La visualización de estas predicciones para la configuración ARMA(1,1) con distribución normal y $\sigma^2=1.0$ se presenta en la Figura~\ref{fig:dist_ecrp}, donde se observa la evolución de las distribuciones predictivas para el caso de Sieve Bootstrap, DeepAR y Block Bootstrap lo largo de seis pasos.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.6\textwidth]{Imagenes/figura_tesis_ecrps.png}
    \caption{Evolución de las distribuciones predictivas para Sieve Bootstrap, DeepAR y Block Bootstrapping en la configuración ARMA(1,1) con distribución normal y $\sigma^2=1.0$.}
    \label{fig:dist_ecrp}
\end{figure}
...
La Figura~\ref{fig:desempeno_escenario} cuantifica el ECRPS promedio por escenario, ordenando los modelos según su desempeño en procesos ARIMA (el escenario más exigente). Lo cual es consistente con la hipótesis de que la no estacionariedad amplifica las diferencias de rendimiento.La clara separación entre grupos de métodos confirma que la no estacionariedad amplifica las diferencias de rendimiento.
...
Los modelos están ordenados por su desempeño en ARIMA (barras grises). El ranking por escenario evidencia que Block Bootstrapping y AREPD experimentan degradación severa en ARIMA (ECRPS $>$ 10), mientras que métodos como Sieve Bootstrap presentan ECRPS entre 0.55 y 0.62 en todos los escenarios y DeepAR entre 0.57 y 4.33. En el escenario SETAR, los métodos convergen hacia un desempeño más homogéneo (ECRPS $\approx$ 0.63-0.70).
...
Los resultados agregados revelan tres hallazgos principales. Primero, los métodos Sieve Bootstrap, LSPM y LSPMW exhiben la mayor estabilidad, alcanzando puntajes Z máximos de 3. Segundo, se observa una marcada heterogeneidad en el desempeño según la configuración: la columna ARIMA(2,1,2) concentra los valores Z más elevados (rojo intenso) para prácticamente todos los métodos, sugiriendo que esta parametrización particular representa un desafío sistemático. Tercero, los métodos LSPM y LSPMW muestran un patrón contrastante, con excelente desempeño en configuraciones ARMA pero deterioro en configuraciones ARIMA específicas.
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{Z-scores de ECRPS por configuración según familia de procesos (continuación).}
...
\subsubsection{\texorpdfstring{Régimen I: Integración Moderada ($d = 1, 2$)}{Régimen I: Integración Moderada (d = 1, 2)}}
...
\subsubsection{\texorpdfstring{Régimen II: Integración Alta ($d = 3, 4, 5$)}{Régimen II: Integración Alta (d = 3, 4, 5)}}
...
\subsubsection{\texorpdfstring{Régimen III: Integración Extrema ($d \geq 6$)}{Régimen III: Integración Extrema (d >= 6)}}
...
\subsubsection{\texorpdfstring{MCPS y AV-MCPS: Necesidad Casi Universal para $d \geq 2$}{MCPS y AV-MCPS: Necesidad Casi Universal para d >= 2}}
...
\subsubsection{\texorpdfstring{LSPM y LSPMW: Robustez Exacta para $d = 1$ y Quiebre en $d = 2$}{LSPM y LSPMW: Robustez Exacta para d = 1 y Quiebre en d = 2}}
...
\subsubsection{\texorpdfstring{Sieve Bootstrap: Transición Gradual con Umbral Efectivo en $d = 5$-$6$}{Sieve Bootstrap: Transición Gradual con Umbral Efectivo en d = 5-6}}
...
Las Figuras~\ref{fig:zscore_arma_tam}-\ref{fig:zscore_setar_tam} desagregan los patrones de convergencia por escenario, revelando que la velocidad de convergencia es altamente dependiente de la estructura del proceso generador.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{\textwidth}
        ...
        \caption{Procesos ARIMA}
        \label{fig:zscore_arima_tam}
    \end{subfigure}
    \caption{Z-scores de ECRPS por configuración según familia de procesos.}
    \label{fig:ecrps_config_parte1_tam}
\end{figure}
...
Las Figuras~\ref{fig:sig_arma_tam}-\ref{fig:sig_setar_tam} desagregan los patrones de significancia por familia de procesos, revelando que la detección de diferencias significativas es altamente dependiente de la estructura del proceso generador.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{\textwidth}
        ...
        \caption{Procesos ARIMA}
        \label{fig:sig_arima_tam}
    \end{subfigure}
    \caption{ECRPS medio y \% de significancia según familia de procesos.}
...
Las Figuras~\ref{fig:zscore_arma_prop}-\ref{fig:zscore_setar_prop} desagregan los Z-scores por escenario, confirmando que la familia de procesos modera fuertemente el régimen de optimalidad.
...
\begin{figure}[htbp]
    ...
    \caption{Procesos ARIMA}
    \label{fig:zscore_arima_prop}
    \end{subfigure}
    \caption{Z-scores de ECRPS por configuración según familia de procesos.}
    \label{fig:ecrps_config_parte1_prop1}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{Z-scores de ECRPS por configuración según familia de procesos.}
    \label{fig:ecrps_config_parte1_prop}
\end{figure}

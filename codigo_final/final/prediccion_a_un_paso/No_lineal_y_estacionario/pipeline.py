...
# ============================================================================
# VERSIÓN PARA MODELOS NO LINEALES (SETAR, TAR, EXPAR, BILINEAR)
# ============================================================================

class PipelineMCPSOnlyNonlinear:
    """Pipeline optimizado SOLO para MCPS y AV-MCPS con modelos no lineales puros."""
    N_TEST_STEPS = 5
    BURN_IN_PERIOD = 50
    ...
    def execute(self, show_intermediate_plots=False):
        """Ejecuta pipeline con simulación no lineal pura."""
        sim_config = {k: v for k, v in self.config.items() 
                     if k not in ['n_samples', 'verbose']}
        ...
        # Plot opcional de la serie completa
        if show_intermediate_plots:
            plots_directory = "plots_densidades_mcps_nonlinear"
            os.makedirs(plots_directory, exist_ok=True)
            series_plot_filename = os.path.join(
                plots_directory,
                f"escenario_{self.scenario_id}_series_completa.png"
            )
            plot_title = f"Serie No Lineal - Escenario {self.scenario_id} ({self.config['model_type']})"
        ...
        for t in range(self.N_TEST_STEPS):
            ...
            if show_intermediate_plots:
                ...
                title = f"MCPS vs AV-MCPS (No Lineal) - Escenario {self.scenario_id} - Paso {t + 1}"
                metrics_for_plot = {k: v for k, v in step_ecrps.items() if k != 'Paso'}
...
class ScenarioRunnerMCPSOnlyNonlinear:
    """Runner exclusivo para MCPS y AV-MCPS con modelos no lineales."""
    ...
    def __init__(self, seed=420):
        ...
        self.distributions = ['normal', 'uniform', 'exponential', 't-student', 'mixture']
        self.variances = [0.2, 0.5, 1.0, 3.0]
        self.plots_directory = "plots_densidades_mcps_nonlinear"

    def _generate_scenarios(self, n_scenarios):
        """Genera todas las combinaciones de escenarios no lineales."""
        all_possible_scenarios = []
        for model in self.models_config:
            for dist in self.distributions:
                for var in self.variances:
    ...
    def _run_single_scenario(self, scenario_dict):
        """Ejecuta un solo escenario no lineal puro."""
        try:
            config = scenario_dict['config']
            seed = scenario_dict['seed']
            plot = scenario_dict.get('plot', False)
            
            pipeline = PipelineMCPSOnlyNonlinear(
                model_type=config['model_type'],
                phi_low=config['phi_low'],
                phi_high=config['phi_high'],
    ...
    def run(self, n_scenarios=140, excel_filename="resultados_mcps_nonlinear.xlsx", 
            batch_size=20, restart_every=5, plot: bool = False):
        """
        Ejecuta todos los escenarios no lineales puros solo con MCPS y AV-MCPS.
        ...
        print(f"\n{'='*70}")
        print(f"🚀 EJECUTANDO {len(all_scenarios_configs)} ESCENARIOS NO LINEALES PUROS")
        print(f"📊 Modelos: MCPS y AV-MCPS únicamente")
        print(f"📦 {n_batches} lotes de máximo {batch_size} escenarios")
        print(f"{'='*70}\n")
        ...
        for batch_num in range(n_batches):
            ...
            if all_excel_rows and (batch_num + 1) % 3 == 0:
                temp_filename = f"checkpoint_mcps_nonlinear_batch_{batch_num + 1}.xlsx"
                pd.DataFrame(all_excel_rows).to_excel(temp_filename, index=False)
                print(f"  💾 Checkpoint guardado: {temp_filename}")
        ...
        if all_excel_rows:
            ...
            ordered_columns = [
                'Paso', 'Tipo de Modelo', 'Phi Régimen Bajo', 'Phi Régimen Alto',
                'Umbral', 'Retardo', 'Distribución', 'Varianza error', 
                'MCPS', 'AV-MCPS', 'Mejor Modelo'
            ]
            df_final = df_final[ordered_columns]
    ...
    def plot_results_from_excel(self, filename):
        ...
        for step_name in pasos_disponibles:
            ...
            title_prefix = (f"MCPS vs AV-MCPS (No Lineal) - Paso {step_name}" 
                          if step_name != "Promedio" 
                          else "MCPS vs AV-MCPS (No Lineal) - Resultados Generales")
...
# Función auxiliar para modelos no lineales
def run_mcps_nonlinear_comparison(n_scenarios=140, plot=False):
    """
    Ejecuta comparación MCPS vs AV-MCPS en modelos no lineales puros.
    
    Args:
        n_scenarios: Número de escenarios (máximo 140)
        plot: Si True, guarda gráficos de densidad
    """
    runner = ScenarioRunnerMCPSOnlyNonlinear(seed=420)
    runner.run(
        n_scenarios=n_scenarios,
        excel_filename="resultados_mcps_vs_av-mcps_nonlinear.xlsx",
        batch_size=20,
        restart_every=5,
        plot=plot
    )
...
# USO NO LINEAL:
# run_mcps_nonlinear_comparison(n_scenarios=140, plot=False)